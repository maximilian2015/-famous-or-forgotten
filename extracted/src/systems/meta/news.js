...
export function respondToPress(s, choice) {
  ...
  if (choice === 'reply') {
    ...
    if (chance(odds)) { s.media = clamp((s.media || 0) + 5); setFame(s, (s.fame || 0) + 1); s.respect = clamp((s.respect || 0) + 1); s.lastEvent = 'You answered the piece. Your reply gets quoted more than the article did.'; addTimeline(s, 'Answered a press story — it landed well.'); }
