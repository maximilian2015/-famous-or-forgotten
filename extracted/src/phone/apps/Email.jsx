...
export function Email({ g }) {
  ...
  return (<div>{box.map((m) => { const chip = m.kind === 'bill' ? ['bill', theme.gold] : m.kind === 'agent' ? ['representation', theme.gold] : m.kind === 'contract' ? ['contract', theme.gold] : m.kind === 'fan' ? ['fan mail', theme.good] : m.kind === 'hate' ? ['forwarded', theme.bad || '#ff5a72'] : m.kind === 'spam' ? ['spam', theme.muted] : m.tag === 'vip' ? ['VIP', theme.good] : ['invite', theme.accent];
