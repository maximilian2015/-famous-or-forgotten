...
export function Email({ g }) {
  ...
  if (openId) { const m = box.find((x) => x.id === openId);
    if (m) return (<div><button onClick={() => setOpenId(null)} style={{ ...btn(), maxWidth: 120, marginBottom: 10 }}>‹ Inbox</button><div style={{ background: theme.panel2, borderRadius: 14, padding: 12 }}><div style={{ fontSize: 11, fontWeight: 900, color: theme.accent, textTransform: 'uppercase' }}>{m.from}</div><div style={{ fontSize: 14, fontWeight: 800, margin: '2px 0 6px' }}>{m.subj}</div><div style={{ fontSize: 13, lineHeight: 1.5, whiteSpace: 'pre-line' }}>{m.body}</div><div style={{ display: 'flex', gap: 7, marginTop: 10 }}>{m.kind === 'contract' && m.tag === 'option'
            ? <button onClick={() => { dispatch((s) => { s.openOption = m.id; return s; }); setOpenId(null); }} style={btn('pri')}>Read the paper</button>
            : (m.cta || []).map((c, i) => (<button key={i} onClick={() => { dispatch(c.tour ? answerTourMail : emailAct, m.id, i); setOpenId(null); }} style={btn(i === 0 ? 'pri' : '')}>{c.label}</button>))}</div></div></div>); }
