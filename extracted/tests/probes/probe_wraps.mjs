...
function live(kind, years = 40) {
  ...
  let wraps = []; let t = s, f55 = null, r30 = null, r40 = null, lastWrap = -99, faceMonths = 0, craftMonths = 0, trail = [];
  for (let m = 0; m < years * 12; m++) {
    ...
    const b = [...T.SCHOOLS].reverse().find((sc) => (t.cash || 0) > sc.cost * 4); if (b && (t.ap || 0) > 1) T.train(t, b.id);
    ...
    const had = !!t.production; const snap = had ? { m: t.production.meter, b: t.production.crew[0].bond, n: t.production.months, left: t.production.monthsLeft, ap: t.ap, w: t.production._workedMonth, stamp: t.year*12+t.month } : null;
    t = advanceMonth(t);
    if (had && !t.production) { lastWrap = m; if (kind === "perfect" && wraps.length < 14) wraps.push(snap); }
    const cb = comboOf(t);
  ...
  return { wraps, f55, r30, r40, faceMonths, craftMonths, trail, credits: (t.filmography || []).length, endF: Math.round(t.fame), endR: Math.round(t.respect) };
}
const med = (a) => (a.length ? [...a].sort((x, y) => x - y)[a.length >> 1] : '—');
for (const kind of ['perfect']) {
  const runs = []; for (let i = 0; i < 2; i++) runs.push(live(kind)); console.log(JSON.stringify(runs[0].wraps));
