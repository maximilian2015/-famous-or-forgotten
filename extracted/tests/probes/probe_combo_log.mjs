...
function live(kind, years = 45) {
  ...
  Object.assign(s, { stage: 'career', ageY: 22, year: 2050, month: 0, hasApartment: true,
    housing: 'room', cash: 9000, _rlog: {}, alive: true, ap: 3, apMax: 3, apMaxEff: 3, fame: 0, peakFame: 0 });
  let t = s, iconAge = null, alistAge = null, lowR = 0, negMonths = 0; const months = {};
  ...
  return { credits: cr.length, peak: Math.round(t.peakFame || 0), alistAge, iconAge,
    ...
    rlog: t._rlog, rcount: t._rcount, months, hit, ledHit, twoHits, nom, died: !t.alive, age: t.ageY, lowR, negMonths, endR: Math.round(t.respect || 0) };
}
...
for (const kind of ['perfect', 'ordinary', 'drifting']) {
  ...
  for (const k of Object.keys(agg).sort((a, b) => agg[b] - agg[a])) console.log('     ' + k.padEnd(40) + (agg[k] / runs.length).toFixed(1).padStart(8) + ' per life  (' + (cnt[k] / runs.length).toFixed(0) + ' times)');
  const seen = (k) => runs.filter((r) => (r.months[k] || 0) > 0).length;
  console.log('  combos: ' + ['beginning','craft','star','real','face','tale'].map((k) => k + ' ' + Math.round(100 * (tot[k] || 0) / all) + '% (' + seen(k) + '/25 lives)').join(' · '));
  ...
  console.log(kind.padEnd(9)
    ...
    + ' · hits ' + runs.filter((r) => r.hits).length + '/25' + '  KEYS: hit85 ' + runs.filter((r) => r.hit).length + '/25 nom ' + runs.filter((r) => r.nom).length + '/25 | LED a hit ' + runs.filter((r) => r.ledHit).length + '/25 | two hits ' + runs.filter((r) => r.twoHits).length + '/25 | led+nom ' + runs.filter((r) => r.ledHit || r.nom).length + '/25');
}
