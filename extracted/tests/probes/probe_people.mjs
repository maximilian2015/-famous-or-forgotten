...
function live(kind, years = 45) {
  ...
  return { people: (t.people||[]).length, warm: (t.people||[]).filter(p=>!p.cold).length, fromSet: (t.people||[]).filter(p=>p.fromSet).length, agentAt: t._agentAt, agent: !!(t.agent && t.agent.level > 0), credits: cr.length, peak: Math.round(t.peakFame || 0), alistAge, iconAge,
