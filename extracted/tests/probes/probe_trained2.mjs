...
function live(kind, years = 40) {
  ...
  Object.assign(s, { stage: 'career', ageY: 22, year: 2050, month: 0, hasApartment: true,
    housing: 'room', cash: 9000, _rlog: {}, acting: kind === 'trained' ? 72 : s.acting, alive: true, ap: 100, apMax: 100, apMaxEff: 100, fame: 0, peakFame: 0 });
  ...
  for (let m = 0; m < years * 12; m++) {
    ...
    const free = kind === 'perfect' || (m - lastWrap) > (kind === 'trained' ? 14 : 24);
  ...
  return { films: (t.filmography||[]).map(c=>[c.scale||c.type, Math.round(c.rating||0), c.tier, c.months||0]), acting: Math.round(t.acting), rlog: t._rlog, rcount: t._rcount, f55, r30, r40, faceMonths, craftMonths, trail, credits: (t.filmography || []).length, endF: Math.round(t.fame), endR: Math.round(t.respect) };
}
const med = (a) => (a.length ? [...a].sort((x, y) => x - y)[a.length >> 1] : '—');
for (const kind of ['trained']) {
  ...
  console.log('   age:fame/standing every 3y —', runs[0].trail.join(' ')); console.log('   acting at end', runs.map(r=>r.acting).join(' ')); const all=runs.flatMap(r=>r.films); const bands={}; for (const f of all){const b=f[1]<45?'<45':f[1]<60?'45-59':f[1]<70?'60-69':f[1]<85?'70-84':'85+'; bands[b]=(bands[b]||0)+1;} console.log('   ratings', JSON.stringify(bands), 'by scale', JSON.stringify(all.reduce((a,f)=>{a[f[0]]=(a[f[0]]||0)+1;return a;},{})), 'months', JSON.stringify(all.reduce((a,f)=>{a[f[3]]=(a[f[3]]||0)+1;return a;},{})));
