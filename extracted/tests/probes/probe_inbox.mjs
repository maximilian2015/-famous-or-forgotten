...
function live(kind, years = 45) {
  ...
  let t = s, iconAge = null, alistAge = null, lowR = 0, negMonths = 0; const months = {}; const texts = {}, mails = {};
  for (let m = 0; m < years * 12; m++) {
    ...
    { const now = t.year * 12 + t.month; for (const m of t.sms || []) if (m.when === now) texts[m.tag] = (texts[m.tag] || 0) + 1; for (const m of t.inbox || []) if (!m._seen) { m._seen = true; mails[m.tag] = (mails[m.tag] || 0) + 1; } }
  ...
  return { texts, mails, agentAt: t._agentAt, agent: !!(t.agent && t.agent.level > 0), credits: cr.length, peak: Math.round(t.peakFame || 0), alistAge, iconAge,
...
for (const kind of ['perfect', 'ordinary', 'drifting']) {
  ...
  const tx = {}, ml = {}; for (const r of runs) { for (const k in r.texts) tx[k] = (tx[k] || 0) + r.texts[k]; for (const k in r.mails) ml[k] = (ml[k] || 0) + r.mails[k]; }
  const yrs = 45 * runs.length; console.log('  texts/yr: ' + Object.keys(tx).map((k) => k + ' ' + (tx[k] / yrs).toFixed(2)).join(' · ') + ' | mail/yr: ' + Object.keys(ml).map((k) => k + ' ' + (ml[k] / yrs).toFixed(2)).join(' · '));
